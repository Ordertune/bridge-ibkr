...
# ── T1-223 Nachtrag: der Halt wird bestaetigt ────────────────────────────────


@pytest.mark.skipif(shutil.which("node") is None, reason="node fehlt")
def test_der_halt_gilt_erst_nach_zwei_fehlschlaegen(tmp_path: Path) -> None:
    """Owner-Befund 2026-09-25: „Stopping the Bridge." blieb fuer immer stehen.

    Im Terminal stand laengst „exited normally"; im Browser eine Verlaufsform,
    die nach einem andauernden Vorgang klingt — ueber vier gruenen Lampen.

    Der Beleg liegt in dem, was dabei schiefgeht: antwortet der lokale Server
    nicht mehr, ist der Vorgang beendet. Er gehoert zur Bridge und stirbt mit
    ihr.

    Gemessen wird hier die eigentliche Entscheidung: **zwei** Fehlschlaege
    hintereinander, nicht einer. Aus einem Aussetzer „beendet" zu machen waere
    genau die Sorte Behauptung ohne Grundlage, die dieser Vorgang behebt.

    Die Uhr wird dafuer ersetzt, nicht abgewartet — eine Zusicherung, die zwei
    Sekunden schlaeft, wird beim naechsten Mal uebersprungen.
    """
    import json

    skript = _skript()
    treffer = re.search(r"^function beobachteHalt\(\).*?^\}", skript, re.S | re.M)
    assert treffer, "beobachteHalt() nicht gefunden"

    datei = tmp_path / "halt.js"
    datei.write_text(
        """
let beendet = false;
let stromZu = false;
const strom = { close() { stromZu = true; } };
const meldungen = [];
function note(id, res) { meldungen.push(res.message); }
function render() {}
function withToken(u) { return u; }

let takt = null;
function setInterval(fn) { takt = fn; return 1; }
function clearInterval() { takt = null; }
let antwort = () => Promise.reject(new Error("weg"));
function fetch() { return antwort(); }
...
(async () => {
  takt(); await warte();
  const nachEinem = beendet;
  takt(); await warte();
  console.log(JSON.stringify({nachEinem, beendet, stromZu,
                              meldungen, taktLaeuft: takt !== null}));
})();
""",
        encoding="utf-8",
    )
    ergebnis = subprocess.run(["node", str(datei)], capture_output=True, text=True)
    assert ergebnis.returncode == 0, ergebnis.stderr
    d = json.loads(ergebnis.stdout)

    assert d["nachEinem"] is False, "Ein einzelner Aussetzer gilt schon als Halt."
    assert d["beendet"] is True, "Nach zwei Fehlschlaegen gilt der Halt nicht."
    assert d["stromZu"] is True, (
        "Der Ereignisstrom bleibt offen — der Browser versucht dann endlos, "
        "ihn gegen einen Server wiederherzustellen, den es nicht mehr gibt."
    )
    assert d["taktLaeuft"] is False, "Die Uhr laeuft weiter, obwohl nichts mehr kommt."
    assert d["meldungen"] == ["The Bridge has stopped."], d["meldungen"]


@pytest.mark.skipif(shutil.which("node") is None, reason="node fehlt")
def test_solange_der_server_antwortet_gilt_kein_halt(tmp_path: Path) -> None:
    """Der Gegenbeweis. Ohne ihn belegt der Test oben nur, dass irgendwann
    „beendet" herauskommt — nicht, dass es an der Unerreichbarkeit liegt."""
    import json
...
const warte = () => new Promise(r => setTimeout(r, 0));
(async () => {
  for (let i = 0; i < 5; i++) { takt(); await warte(); }
  console.log(JSON.stringify({beendet}));
})();
""",
        encoding="utf-8",
    )
    ergebnis = subprocess.run(["node", str(datei)], capture_output=True, text=True)
    assert ergebnis.returncode == 0, ergebnis.stderr
    assert json.loads(ergebnis.stdout)["beendet"] is False
